...
And my session should be securely created  
...
When I try to access the dashboard URL  
Then I should be redirected to the login page  
```
...
And it should appear in the doctor list  

Given I am on the "Add Doctor" form  
When I submit incomplete or invalid information  
Then I should see validation errors  
And the doctor profile should not be saved  
```
...
Then the doctor record should be removed from the system  

Given a doctor has scheduled future appointments  
When I attempt to delete the doctor  
...
```gherkin
Given I am logged in as an admin  
When I request monthly appointment statistics  
Then the system should execute the appointment_stats procedure  
And display appointment counts grouped by month  

Given there is no appointment data  
When I request the report  
Then the system should display an empty result or message indicating no data  
```
